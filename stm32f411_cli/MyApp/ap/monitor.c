...
static monitor_mode_t current_monitor_mode = MONITOR_MODE_ASCII;
...
static void cliMonitor(uint8_t argc, char** argv){
    ...
    if(current_monitor_mode == MONITOR_MODE_ASCII){
        LOG_INF("Current_ Mode : [ASCII]");
    } else LOG_INF("Current Modde : OFF");
...
void monitorSendPacket(void){
    ...
    if (current_monitor_mode == MONITOR_MODE_ASCII) {
      ...
      // 시작 문자 $
      len += snprintf(tx_buf + len, sizeof(tx_buf) - len, "$%d ",
                      g_packet.count); // int 위치 정보 리턴
      for (int i = 0; i < g_packet.count; i++) {
